...
import { Data, Effect, List, Request, Schedule, pipe } from "effect";
...
const mailer = Machine.makeWith<List.List<SendEmail>>()((_, previous) =>
	Effect.gen(function* () {
		...
		if (List.isCons(state)) {
			yield* ctx
				.unsafeSend(new ProcessEmail())
				.pipe(Effect.replicateEffect(List.size(state)));
		}
			...
			Machine.procedures.addPrivate<ProcessEmail>()(
				...
				({ state }) =>
					Effect.gen(function* () {
						...
						yield* Effect.log(`Sending email to ${req.email}`).pipe(
							Effect.delay(500),
						);
...
pipe(program, Effect.scoped, runMain);
